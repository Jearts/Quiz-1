...
def check_winner():
    ...
    for row in range(3):
        if (board[row][0]["text"] == board[row][1]["text"] == board[row][2]["text"]
            ...
            label.config(text=board[row][0]["text"]+" was won the game yipeeee!🎉🎉🎉", foreground=color_yellow)
    ...
    for column in range(3):
        if (board[0][column]["text"] == board[1][column]["text"] == board[2][column]["text"]
            ...
            label.config(text=board[0][column]["text"]+" was won the game yipeeee!🎉🎉🎉", foreground=color_yellow)
            for row in range(3):
                board[row][column].config(foreground=color_yellow, background=color_light_gray)
            game_over = True
            return
    ...
    if (board[0][0]["text"] == board[1][1]["text"] == board[2][2]["text"]
        and board[0][0]["text"] != ""):
        label.config(text=board[0][0]["text"]+" was won the game yipeeee!🎉🎉🎉", foreground=color_yellow)
        for i in range(3):
            board[i][i].config(foreground=color_yellow, background=color_light_gray)
        game_over = True
        return

    #anti-diagionally tohh
    if (board[0][2]["text"] == board[1][1]["text"] == board[2][0]["text"]
        and board[0][2]["text"] != ""):
        label.config(text=board[0][2]["text"]+" was won the game yipeeee!🎉🎉🎉!", foreground=color_yellow)
    ...
    if (turns == 9):
        game_over = True
        label.config(text="its atay! wait.. i mean a tie hehe", foreground=color_yellow)
...
for row in range(3):
    for column in range(3):
        board[row][column] = tkinter.Button(frame, text="", font=("Consolas", 60, "bold"),
                                            background=color_gray, foreground=color_blue, width=6, height=1,
...
#para i center yung window hehehe
window.update()
